    ...
    def date_insert(self,item,db_name,selfcol):
        ...
        for i in xrange(0, len(memory[0])):
            try:
                ...
                pass
    def __init__(self):
        self.conn = MySQLdb.connect(host="211.83.111.10", user="Kbase", passwd="123456", db="kbase", port=4306,
                                    charset="utf8")
...
'news_url',
        ...
        self.chattel_mortgage = ['registed_num',
                                 ...
                                 'vouched_type',
                                 'vouched_amount',
                                 'debt_deadline',->debt_start,debt_end
                                 'vouched_range',
                                 备注，
                                 担保物(名称+所有权归属+数量、质量、状况、所在地等情况+备注)]
    ...
    def process_item(self, item, spider):
        ...
        for i in xrange(len(self.company)):
            memory.append(item[self.company[i]][0])
        try:
         self.cursor.execute(
            "INSERT INTO company VALUES(NULL,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ", tuple(memory))
        except:
         self.cursor.execute(
         ...
         self.conn.commit()
         self.cursor.execute("SELECT companyid FROM company WHERE tyqycid='%s'"%item["company_id"][0])
         item["company_id"]=int(self.cursor.fetchone()[0])
        ...
        # for (x,y,i),in (item["shareholder_name"],item["shareholder_id"],len(memory[0])):
        ...
        """investmentinfo"""
        invested_fk=[]
        for (x, y), in (item["invested_company_name"], item["invested_company_id"]):
                selec_result = self.cursor.execute("SELECT companyid FROM company WHERE tyqycid='%s'" % y)
                if selec_result == 0:
                    self.cursor.execute("INSERT INTO company(tyqycid,comp_name) VALUES (%s,%s)", (y, x))
                    invested_fk.append(int(self.cursor.lastrowid))
                else:
                    invested_fk.append(int(self.cursor.fetchone()[0]))
        self.conn.commit()
        ...
        for i in xrange(0, len(memory[0])):
            self.cursor.execute("INSERT INTO investment "
                                "VALUES(NULL,"+item["company_id"]+","
                                "%s,%s,%s,%s,%s,%s,%s,%s)",
                                tuple(x[i] for x in memory))
        ...
        for i in xrange(0, len(memory[0])):
            self.cursor.execute("INSERT INTO judgement "
                                ...
                                tuple(x[i] for x in memory))
            judge_fk.append(int(self.cursor.lastrowid))
        ...
        for i in xrange(0, len(memory[0])):
            self.cursor.execute("INSERT INTO lawsuit "
                                "VALUES(NULL,"+item["company_id"]+","
                                "%s,%s,%s)",
                                tuple(x[i] for x in memory))
        ...
        for i in xrange(0, len(memory[0])):
            self.cursor.execute("INSERT INTO RFP "
                                "VALUES(NULL,%s,%s,%s,%s)",
                                tuple(x[i] for x in memory))
            rfp_fk.append(int(self.cursor.lastrowid))
        self.conn.commit()
